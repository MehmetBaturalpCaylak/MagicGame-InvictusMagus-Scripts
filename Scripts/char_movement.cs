...
public class char_movement : MonoBehaviour
{
    ...
    //This one will give us layer of p2
    public LayerMask p2_layer;
    ...
    void Start()
    {
        ...
        */
        ...
        p1 = gameObject.GetComponent<char_oop>();
        ...
        speed = p1.getSpeed();
        jumpPower = p1.jumpPower;
        exjumptime = p1.exJump;
    ...
    private void FixedUpdate()
    {
        ...
        if (dashMove)
        {
            ...
            if (p1.charID == 2)
            {
                GameObject.Find("ID2DashEffectSpawner").GetComponent<BurningEffect>().spawnfireObject(gameObject.transform, "p2");
            }
            if (dashTime == 5)
            {
                hitable = true;
    ...
    private void Dash()
    {
        ...
        if (lastcooldowndash <= 0)
        {
            if (canDash)
            {
                ...
                dashMove = true;
                canMove = false;
                lastcooldowndash = p1.cooldownDash;
            }
        }
    }
    private void Attack()
    {
        if (canAttack)
        {
            ...
            {
                lastcooldownofattack = p1.cooldownAttack;
                attackKeyPressed = true;

                Transform attP = atackP.transform;
                Collider2D p2finder = Physics2D.OverlapBox(attP.position, attP.localScale, 0, p2_layer);
                switch (p1.getID())
                {
                    case 1:
                        startSkid(0.16f);
                ...
                float direction = p2finder.transform.position.x - gameObject.transform.position.x;
                //********************************************************************************
                if (direction < 0)                                                             //*
                {                                                                              //*
                    direction = -1;                                                            //*
                ...
                if (p2finder.GetComponent<char_movement2>().getHittable())
                {
                    switch (p1.charID)
                    {
                        case 1:
                            p2finder.GetComponent<char_movement2>().startSkid(0.6f);
                            p2finder.GetComponent<Rigidbody2D>().AddForce(new Vector2(20 * direction, 10), ForceMode2D.Impulse);
                            p2finder.GetComponent<char_oop2>().slowit();
                            p2finder.GetComponent<char_oop2>().setHP(-15);
                            break;
                        case 2:
                            p2finder.GetComponent<char_movement2>().startSkid(0.6f);
                            p2finder.GetComponent<Rigidbody2D>().AddForce(new Vector2(20 * direction, 10), ForceMode2D.Impulse);
                            p2finder.GetComponent<char_oop2>().burnit();
                            p2finder.GetComponent<char_oop2>().setHP(-10);
                            break;
                        case 3:
                            p2finder.GetComponent<char_movement2>().startSkid(0.6f);
                            p2finder.GetComponent<Rigidbody2D>().AddForce(new Vector2(25 * direction, 15), ForceMode2D.Impulse);
                            p2finder.GetComponent<char_oop2>().setHP(-15);
                            break;
                        default:
                            break;
                ...
                {
                    if (p2finder.GetComponent<char_movement>().counterAttackbool)
                    {
                        startSkid(0.8f);
                    ...
                    else
                    {
                        startSkid(0.2f);
                        gameObject.GetComponent<Rigidbody2D>().AddForce(new Vector2(-5 * direction, 2.5f), ForceMode2D.Impulse);
                        GameObject.FindGameObjectWithTag("spawnerblock").GetComponent<BlockDonePopUpScript>().spawnTemp(p2finder.transform);
                    }
                }
            }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if(collision.gameObject.tag == "p2")
        {
            if (dashMove)
            {
                if (p1.charID == 3)
                {
                    collision.gameObject.GetComponent<char_movement2>().startSkid(0.5f);
